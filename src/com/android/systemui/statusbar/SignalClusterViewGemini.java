...
public class SignalClusterViewGemini extends LinearLayout implements NetworkControllerGemini.SignalCluster {
    ...
    public String im1,im2;//add by hjz
    private ImageView mobile_signal_x;
    ...
    public SignalClusterViewGemini(Context context, AttributeSet attrs, int defStyle) {
        ...
        Log.d("hjz","mMobileStrengthIdNu"+mMobileStrengthIdNum+"mGeminiSimNum="+mGeminiSimNum);
    ...
    protected void onAttachedToWindow() {
        ...
        mobile_signal_x= (ImageView) findViewById(R.id.mobile_signal_x);
        ln1 = (LinearLayout) findViewById(R.id.signal_cluster_combo_2);
        ...
        Log.d("kay21", "onAttachedToWindow: mGeminiSimNum:" + mGeminiSimNum);
    ...
    public void apply() {
        ...
        for(int i = 0; i < mGeminiSimNum ; i++) {
            ...
            Log.d("hjz","ka1="+shouldMobileGroupVisible(0)+"ka2="+shouldMobileGroupVisible(1));
            //Xlog.d("hjz", "apply(), slotId=" + i +", mMobileVisible=" + mMobileVisible[i] + ", mShouldMobileGroupVisible=" + mShouldMobileGroupVisible);
           // Xlog.d("hjz", "apply(), slotId=" + i +", StrengthId=" + mMobileStrengthId[i][0].getIconId());
            if (mMobileVisible[i] && mShouldMobileGroupVisible) {
                ...
                if(false){
                   // boolean shouldmode = shouldMobileGroupVisible(x);
                }
                //begin add by hjz
                if (mMobileStrengthId[i][0].getResources() != null) {
                    Log.d("hjz","1setImageDrawable");
                    mMobile[i].setImageDrawable(mMobileStrengthId[i][0].getDrawable());
                } else {
                    if (mMobileStrengthId[i][0].getIconId() == 0) {
                        Log.d("hjz","2=2=2="+mMobileStrengthId[i][0]);
                        mMobile[0].setImageResource(mMobileStrengthId[1][1].getIconId());
                        mMobile[0].setPadding(0, 10, 0, 0);
                        mMobile[1].setVisibility(View.INVISIBLE);
                    } else {
                        Log.d("hjz","3and i="+i+"mMobileStrengthId[0][0].getIconId()="+mMobileStrengthId[0][0].getIconId()+"2="+mMobileStrengthId[1][1].getIconId());
                        //Log.d("hjz","mGeminiSimNum="+mGeminiSimNum);
                        mMobile[0].setImageResource(mMobileStrengthId[i][0].getIconId());
                        mMobile[0].setPadding(0, 0, 0, 0);
                        //mMobile[1].setImageResource(2130838284);
                        //mMobile[1].setImageResource(mMobileStrengthId[1][1].getIconId());//add by hjz
                        if(!(shouldMobileGroupVisible(0))&&shouldMobileGroupVisible(1)){
                            //mMobile[0].setPadding(0, 0, 30, 0);
                            //LinearLayout.LayoutParams lp = (LayoutParams) mobile_signal_x.getLayoutParams();
                            ln1.setVisibility(View.VISIBLE);
                            mobile_signal_x.setImageResource(mMobileStrengthId[i][0].getIconId());
                            //mobile_signal_x.setPadding(0, 0, 0, 5);
                        }
                        if(shouldMobileGroupVisible(0)&&shouldMobileGroupVisible(1)){
                            ln1.setVisibility(View.GONE);
                        }
                        if(shouldMobileGroupVisible(1)){
                            Log.d("hjz","mMobile[1].setVisibility");
                            mMobile[1].setImageResource(mMobileStrengthId[1][1].getIconId());
                            mMobile[1].setVisibility(View.VISIBLE);
                        }else{
                            Log.d("hjz","mMobile[1].GONE");
                            mMobile[1].setVisibility(View.INVISIBLE);
                        }
                ...
                if (NetworkType.Type_1X3G != mDataNetType[i]) {
                    Log.d("hjz","mMobile2[i].setVisibility(View.GONE)");
                ...
                if (SIMHelper.isSimInserted(i)
                    ...
                    Log.d("hjz1","state="+state+"simColorId="+simColorId);
                    if (simColorId > -1 && simColorId < 4 && mDataNetType[i] != null) {
                        ...
                        Log.d("hjz1","id="+id+"resource id="+PluginFactory.getStatusBarPlugin(mContext).getPluginResources());
                        if (id != -1) {
                            resId.setResources(PluginFactory.getStatusBarPlugin(mContext).getPluginResources());
                            resId.setIconId(id);
                        }
                        Log.d("hjz1","resId.getResources() != null==="+resId.getResources());
